...
Get-Mailbox | Get-MailboxFolderPermission | Where-Object {($_.user -like 'Anonymous') -or ($_.user -like 'Default') -and ($_.AccessRights -ne 'None') } | fl
...
ForEach ($record in $logs){
$AuditData = $record.AuditData | ConvertFrom-Json
if ( $AuditData.Parameters | Where-Object {
    ($_.Value -like ''Anonymous'') -or ($_.Value -eq'Default') }) {$record}}
